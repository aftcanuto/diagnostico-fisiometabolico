...
function silhueta(sexo: 'M' | 'F', pctG?: number | null, imc?: number | null): string {
  const visual = classificarComposicaoCorporal({ sexo, pctGordura: pctG, imc });
...
function pgResumo(d: LaudoData): string {
  ...
  const pctG   = gorduraRelatorio.valor;
  ...
  const statusLbl = composicaoVisual.label;
...
function pgBio(b: any, ia?: any, gorduraRelatorio?: any): string {
  ...
  const mostrarGorduraBio = b.percentual_gordura != null && (!gorduraRelatorio?.fonteDefinida || gorduraRelatorio.fonte === 'bioimpedancia');
  ...
  <div class="dark-block">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px">
      ...
      ${mostrarGorduraBio?`<div><div style="font-size:20px;font-weight:800;color:#f87171;line-height:1">${b.percentual_gordura}%</div><div style="font-size:8px;color:#6b7280;margin-top:3px">% Gordura</div></div>`:''}
...
function pgPlanoAcao(d: LaudoData, pri: string): string {
  ...
  return quebrarTextoPdf(plano, 2100).map((parte, idx) => pgModulo('Plano de ação', null, `
    ${idx > 0 ? `<div style="font-size:11px;color:#94a3b8;font-style:italic;margin-top:-10px;margin-bottom:18px">continuação</div>` : ''}
    <p style="font-size:12px;color:#64748b;line-height:1.6;margin-bottom:14px">Prioridades, metas e recomendações para a próxima etapa.</p>
...
export function renderLaudoHTML(d: LaudoData): string {
  ...
  const forcaPreensao = scoreForcaPorDadosPreensao(d.dados.forca, d.paciente.sexo, d.paciente.idade);
